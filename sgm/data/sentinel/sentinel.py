...
class SEN12MSCRTSInterface(SEN12MSCRTS):
    ...
    def __getitem__(self, pdx):
        ...
        if self.preprocess:
            img = np.moveaxis(np.stack(target_S2, axis=0).squeeze(0),0,-1)
            auxiliary_S1_img =np.moveaxis(np.stack(in_S1, axis=0),1,-1)
            auxiliary_S2_img =np.moveaxis(np.stack(in_S2, axis=0),1,-1)
            auxiliary_img = np.concatenate((auxiliary_S1_img,auxiliary_S2_img),axis=-1)
            ...
            # mask: (T, H, W)
            mask_unsq = (mask.unsqueeze(1)).float()                  # (T, 1, H, W)
            # print(type(mask_unsq[0,0,0,0]))
            mask_expanded = mask_unsq.expand_as(auxiliary_S2_img)
            # 原始 S
            target_exp = img.unsqueeze(0)        # (1, c, h, w)
            mu = mask_unsq * auxiliary_S2_img + (1.- mask_unsq) * target_exp
            S2_original = mu                     # (T, C2, H, W)

            # 云区设置为 1
            S2_masked = S2_original.clone()
            S2_masked[mask_expanded>0.5] = 1.0                    # 云区=1，无云区保持
            # print(S2_masked)
            # 拼接： [S2原始, mask, S2_masked]
            S2_combined = th.cat([S2_original*2.-1., mask_unsq.float(), S2_masked*2.-1.], dim=1)  # (T, 2*C2+1, H, W)
        t_i, c_i, h_i, w_i = auxiliary_S2_img.shape
        mask_exp = (mask.unsqueeze(1)).float()           # (t, 1, h, w)
        target_exp = img.unsqueeze(0)        # (1, c, h, w)
        mu = mask_exp * auxiliary_S2_img + (1 - mask_exp) * target_exp
        perms = list(itertools.permutations(range(t_i)))
        aux_permuted_list = [mu[list(p), :, :, :] for p in perms]
        S2_perm_list = [S2_combined[list(p), :, :, :] for p in perms]

        # 拼接成 (t*6, c, h, w)
        aux_permuted_tdim = th.cat(aux_permuted_list, dim=0)
        S2_combined_tdim = th.cat(S2_perm_list, dim=0)
        aux_rev_s2 = mu.flip(dims=[0])  # (t, c, h, w)
        aux_double = th.cat([mu, aux_rev_s2], dim=0)  # (2*t, c, h, w)

        target_double = th.cat([img, img], dim=0)  # (2, c, h, w) → 如果需要 (2*t,c,h,w)，需扩展
        s2_combine_aux = S2_combined.flip(dims=[0])
        S2_combined_double = th.cat([S2_combined, s2_combine_aux], dim=0)  # (2*t, c, h, w)
        if self.return_target:
            if self.rescale:
                return {
                    "target" : img * 2. - 1,
                    "target_double" : target_double * 2. - 1,
                    "S1": auxiliary_S1_img * 2. - 1.,
                    "S2": auxiliary_S2_img * 2. - 1.,
                    "S2_double":aux_double* 2. - 1.,
                    "S2_tdim": aux_permuted_tdim *2.-1.,
                    "S1S2": auxiliary_img * 2. - 1.,
                    "dates": dates,
                    "masks": mask,
                    "S2_combined": S2_combined,
                    "S2_combined_double":S2_combined_double,
                    "S2_combined_tdim":S2_combined_tdim, 
