...
class Mixed_Preception(TimestepBlock):
    def __init__(
        ...
        self.attention_layernorm = normalization(patch_channels)
        self.out_features = patch_channels
        self.globalattention = GlobalTransformer(
            channels = self.out_features, use_checkpoint=use_checkpoint,
    ...
    def __init__(
                ...
                if _ == num_res_blocks-1 and level == len(channel_mult) - 1:
                    layers.append(
                        ...
                        if perception_name == 'local'
                                              ...
                                              num_heads = num_heads,patch_size=patch_size,patch_mult=self.patch_mult,
                                              emb_channels = time_embed_dim,
                                              num_head_channels = num_head_channels,use_checkpoint=use_checkpoint,
                                              use_new_attention_order=use_new_attention_order)
                    )
                    ch = ch
        ...
        self.middle_block = TimestepEmbedlevel([
            Mixed_Preception(dims=dims,img_size =img_size,channels = ch,
                                              num_heads = num_heads,patch_size=middle_attention_patch,patch_mult=self.patch_mult,
                                              emb_channels = time_embed_dim,
                                              num_head_channels = num_head_channels,use_checkpoint=use_checkpoint,
