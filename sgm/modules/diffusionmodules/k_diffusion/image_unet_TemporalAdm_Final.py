...
class Temporal_FusionBlock(nn.Module):
    def __init__(
        ...
        self.self_large = TemporalLevel([conv_nd(dims, in_features,patch_channels, 3, padding=1)])
        
        self.self_attention = LTAELayer(d_model=patch_channels,n_head=1,d_k=d_k,dropout=dropout,sequence_len=sequence_len, mlp_conv=[patch_channels,patch_channels*2])
        self.self_project = nn.Sequential(
            conv_nd(dims,patch_channels,out_features,3,padding = 1),
    ...
    def forward(self,x,k_cond,v_cond,emb):
        x1 = self.patchify(x)
        x2 = self.self_large(x,emb)
        sz_b,seq_len, d ,h,w= x1.shape
        out = self.cross_attention(x1.contiguous().view(sz_b,h,w,seq_len,d),k_cond,v_cond)
        # print(out.shape)
        ...
        out = self.ff(out)
        out1 = out+skip
        out2,k_v,v_v = self.self_attention(x2)
        out2 = self.self_project(out2)
        out_final = out1 * self.v + out2*(1.-self.v)
        return out_final
