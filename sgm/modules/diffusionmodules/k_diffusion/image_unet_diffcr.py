...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.intro = nn.Conv2d(in_channels=in_channels, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True)
        self.cond_intro = nn.Conv2d(in_channels=in_channels, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True)
    ...
    def forward(self, x_cond, timesteps, dates=None, return_attn=False):
        t = self.map(self.emb(timesteps.view(-1, )))
        b, seq , c, h, w = x_cond.shape
        x_cond = x_cond.view(b, seq * c, h, w) 
        x_cond = self.check_image_size(x_cond)
        # print(x_cond.size())
        x1, x2, x3 = x_cond.chunk(3, dim=1)
        x = (x1 + x2 + x3) /3 # just try
        # print(x.shape)
        #x1, x2, x3, x = x_cond.chunk(4, dim=1)
        cond = torch.stack([x1, x2, x3], dim=1)
        #cond = torch.cat([x1, x2, x3], dim=1)
        # print(cond.shape)
