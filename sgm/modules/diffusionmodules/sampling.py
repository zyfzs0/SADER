...
class TemporalResidualEulerEDMSampler_MAE_mask(TemporalResidualEulerEDMSampler):
    def __init__(
        self, mae_config=None, repaint_steps = 0, repaint_schurn = 0.0,threshold_diff = 0.01, *args, **kwargs
    ):
        super().__init__(*args, **kwargs)
    ...
    '''
    def maerepaint_step_inner(self, sigma, next_sigma, denoiser, x,x0,x_mu, mu, cond, uc=None, gamma=0.0,eps_d = 1e-8,x_mask=None):
        st = self.sigma2st(sigma)
        st_bc = append_dims(st, x.ndim)
        sigma_bc = append_dims(sigma, x.ndim)
        sigma_hat = sigma * (gamma + 1.0)
        ...
        x_temp_mask = (x_temp > 0)  # bool mask
        if x_mask is not None:
            x_mask_expanded = x_mask.unsqueeze(2).repeat(1, 1, x_temp.shape[2], 1, 1)
            x_temp_mask = (x_mask_expanded > 0.5)
        # 初始化最终mask
        mask = torch.zeros_like(x0)  # (b, seq, c, h, w)
    ...
    def repaint_step(self, glb_step,num_sigmas,sigma, next_sigma, denoiser, x, mu, cond, uc=None , gamma =0.0, 
                     return_attn=False,target=None,mask=None,x_mask=None):
        #print(sigma)
        st = self.sigma2st(sigma)
        st_bc = append_dims(st, x.ndim)
        sigma_bc = append_dims(sigma, x.ndim)
        ...
        if target is not None and mask is not None:
            ...
            new_target = target
            # 融合：mask=1 → 用 target；mask=0 → 用 denoised
            base_denoised = base_denoised * (1. - new_mask) + new_target * new_mask
        x_next = x
        # st0 = self.sigma2st(sigmas[0])
        # # x = mu + sigmas[0] * st * x
        # x = ((1 - st0) / st0) * mu + sigmas[0] * x  
    ...
    def __call__(self, denoiser, x, mu, cond, uc=None, num_steps=None, return_intermediate=False, return_denoised=False, return_attn=False,target=None,mask=None):
        ...
        if target is not None and mask is not None:
            # 生成单通道 new_mask：任意时相无云即为1
            b_i,t_i,h_i,w_i = mask.shape
            p = list(itertools.permutations(range(t_i)))
            masks_perm = [mask[:, list(perm), :, :] for perm in p]  # 每个都是 (b,3,h,w)
            mask_new = torch.cat(masks_perm, dim=1)
            mask =mask_new.clone()
            new_mask = (~mask.bool()).all(dim=1, keepdim=True).float()  # (b, 1, h, w)
            x_mask = mask.float()
