    ...
    def log_local(
        ...
        for k in images:
            if isheatmap(images[k]):
                ...
                if grid.shape == (1, 1, 27) and grid.dtype == np.uint8:
                    print(f"跳过不支持的图像格式: shape={grid.shape}, dtype={grid.dtype}")
                    return  # 直接返回，不执行后续的图像保存
                if grid.dtype != np.uint8:
                    grid = grid.astype(np.uint8)
                img = Image.fromarray(grid[...,:3])
                img.save(path)
                if exists(pl_module):
                    assert isinstance(
...
if __name__ == "__main__":
    ...
    try:
        ...
        os.makedirs(os.path.join(logdir, default_logger_cfgs["csv"]["params"]["name"]), exist_ok=True)
        ...
        if "strategy" in lightning_config:
            ...
            default_strategy_config["params"] = {
                "find_unused_parameters": False,  # ORIGIN: False
                # "static_graph": True,
